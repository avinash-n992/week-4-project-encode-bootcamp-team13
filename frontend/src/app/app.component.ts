...
export class AppComponent {
  ...
  constructor(private http: HttpClient) {
    ...
    this.provider = new ethers.providers.AlchemyProvider('goerli', '_gYR7U4PqlhtlwPpExP-ntINoyOTqchc');
    this.accounts = [];
  ...
  async connectWallet(privateKey: string) {
    try {
      ...
      const balance = this.userWallet.getBalance().then((balanceBN)=> {
        const balanceString = utils.formatEther(balanceBN);
        this.userEthBalance = parseFloat(balanceString);
      })

      // const accounts = await this.provider.listAccounts()

      const accs = await this.userWallet.
